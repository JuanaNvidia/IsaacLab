...
"""This script demonstrates sinusoidal oscillation of a finger using file-based communication.

This script saves joint positions to a file that can be read by a separate real hand controller.

.. code-block:: bash

    # Usage
    ./isaaclab.sh -p scripts/tutorials/02_scene/inspire_sin_scene_file.py --num_envs 1
...
parser = argparse.ArgumentParser(description="Tutorial on sinusoidal finger oscillation with file communication.")
parser.add_argument("--num_envs", type=int, default=1, help="Number of environments to spawn.")
parser.add_argument("--output_file", type=str, default="/tmp/inspire_hand_data.json", help="Output file for joint data")
...
class SinusoidalFingerUI:
    ...
    def __init__(self, num_joints: int = 12):
        ...
        # Finger mapping to real hand (simulation index -> real hand index)
        self.sim_to_real_finger_map = {
            0: 3,  # index_proximal -> Index finger
            1: 2,  # middle_proximal -> Middle finger  
            2: 0,  # pinky_proximal -> Little finger
            3: 1,  # ring_proximal -> Ring finger
            4: 5,  # thumb_yaw -> Thumb rotate
            9: 4   # thumb_pitch -> Thumb bend
        }
    ...
    def get_joint_positions(self):
        ...
        # Set all joints to default except the selected finger
    ...
    def get_real_hand_data(self):
        """Get data to send to real hand controller."""
        current_time = time.time() - self.start_time
        ...
        # Convert simulation position (0-1.5) to real hand position (0-1000)
        real_hand_position = (oscillating_position / 1.5) * 1000
        real_hand_position = max(0, min(1000, real_hand_position))
        
        # Map simulation finger to real hand finger
        real_finger_index = self.sim_to_real_finger_map.get(self.selected_finger, 0)
        
        return {
            "timestamp": time.time(),
            "simulation_finger_index": self.selected_finger,
            "real_finger_index": real_finger_index,
            "frequency": self.oscillation_speed,
            "amplitude": self.oscillation_amplitude,
            "center_position": self.oscillation_offset,
            "sim_position": oscillating_position,
            "real_position": real_hand_position,
            "finger_name": list(self.finger_options.keys())[list(self.finger_options.values()).index(self.selected_finger)]
        }
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    # Print header
    print("\n" + "="*80)
    print("JOINT POSITIONS MONITOR (File Output)")
    print("="*80)
    ...
    while simulation_app.is_running():
        ...
        # Save data to file every 5 steps (for real hand communication)
        if count % 5 == 0:
            try:
                ...
                # Get real hand data
                real_hand_data = finger_ui.get_real_hand_data()
                real_hand_data["actual_sim_positions"] = current_joint_pos.cpu().numpy().tolist()
                real_hand_data["step_count"] = count
                
                # Write to file atomically
                temp_file = args_cli.output_file + ".tmp"
                with open(temp_file, 'w') as f:
                    json.dump(real_hand_data, f, indent=2)
                os.rename(temp_file, args_cli.output_file)
...
def main():
    ...
    print(f"[INFO]: Writing joint data to: {args_cli.output_file}")
    # Run the simulator
